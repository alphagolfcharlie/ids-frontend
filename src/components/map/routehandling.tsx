  ...
  const plotWaypoints = async (
    ...
    try {
      const res = await fetch(
        `https://api.alphagolfcharlie.dev/ids/fix?fixes=${fixes.join(",")}`
      )
...
async function expandProcedure(fixes: string[]): Promise<string[]> {
  ...
  for (let i = 0; i < fixes.length; i++) {
    ...
    if (isProcedure && i === 0 && fixes[i + 1]) {
      ...
      try {
        const res = await fetch(`https://api.alphagolfcharlie.dev/ids/sid?code=${sidCode}`)
    ...
    if (isProcedure && i > 0) {
      ...
      expanded.pop()
      try {
        const res = await fetch(`https://api.alphagolfcharlie.dev/ids/star?code=${starCode}`)
        const data = await res.json()
        expanded.push(...(data.waypoints || [current]))
      } catch {
    ...
    if (isAirway && i > 0 && i < fixes.length - 1) {
      ...
      try {
        const res = await fetch(
          `https://api.alphagolfcharlie.dev/ids/airway?id=${current}&from=${prev}&to=${next}`
        )
        const data = await res.json()
